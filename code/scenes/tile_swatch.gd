...
func set_active_shader():
	Screen.print("set_active_shader()")
	if Global.selected_swatch_index == swatch_index:
		...
		swatch_texture.set_instance_shader_parameter("Brightness",0.75)
	elif swatch_texture.get_instance_shader_parameter("Brightness") != 0:
		Screen.print(str("de-activating shader for ",swatch_index),Color(0.984, 0.457, 0.251, 1.0))
		swatch_texture.set_instance_shader_parameter("Brightness", 0.0)
